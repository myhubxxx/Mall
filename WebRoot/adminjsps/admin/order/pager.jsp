...
<script type="text/javascript">
	function _go() {
		...
		location = "<c:url value='/listOrdersAdmin.action?status=${status}&pageNow='/>" + pc;
	}
</script>
...
<c:choose>
	...
	<c:otherwise><a href="<c:url value='/listOrdersAdmin.action?status=${status}&pageNow='/>${page.currentPage-1}" class="aBtn bold">上一页</a></c:otherwise>
</c:choose>
...
<%--  <c:choose>
 ...
 <c:forEach begin="${page.start }" end="${page.end }" var="i">
   <c:choose>
   	  ...
   	  <c:otherwise>
   	    <a href="<c:url value='/listOrdersAdmin.action?status=${status}&pageNow='/>${i}" class="aBtn">${i }</a>
   	  </c:otherwise>
   </c:choose>
...
<c:choose>
	...
	<c:otherwise><a href="<c:url value='/listOrdersAdmin.action?status=${status}&pageNow='/>${page.currentPage+1}" class="aBtn bold">下一页</a></c:otherwise>
</c:choose>
